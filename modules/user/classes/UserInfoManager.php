...
class UserInfoManager{
	...
	public function getUserInformation($uid){
		...
		//$info->setValue('pic',$info->getValue('upic'));
		...
		$coordinates = $this->cread->getUserCoordinates($uid);
		if(isset($coordinates['lat']))	$info->setObject($coordinates);
		return $info->getInfo();
	}
	...
	public function bookNow($params){
		$booking = new Booking();
		//$bookings = $this->cread->getList($booking,array('seller'=>$params['seller'],'customer'=>$params['customer'],'status'=>0));
		$booking = $this->getActiveBookingByUids($params['seller'],$params['customer']);
		if($booking['id']==0){
			$user = $this->getUserByUid($params['seller']);
			$params['availability'] = $user->getValue('availability');
			$params['locations'] = $user->getValue('cafes');
			$booking = new Booking();
	...
	public function getBookings($uid){
		$booking = new Booking();
		$list1 = $this->cread->getBookingsBySeller($uid);
		$list2 = $this->cread->getBookingsByCustomer($uid);
		$list = array_values(array_merge($list1,$list2));
		usort($list, function ($item1, $item2) {
		    return $item1['suggestedAvailability'] <=> $item2['suggestedAvailability'];
		});

		$bookings = $this->getBookingObjectsFromList($list,$uid);
		return $bookings;
	}

	public function getActiveBookingByUids($uid1,$uid2){
		$booking = new Booking();
		$list1 = $this->cread->getBookingsByUsers($uid1,$uid2);
		$list2 = $this->cread->getBookingsByUsers($uid2,$uid1);
		$list = array_values(array_merge($list1,$list2));
		usort($list, function ($item1, $item2) {
		    return $item1['suggestedAvailability'] <=> $item2['suggestedAvailability'];
		});
		$bookings = $this->getBookingObjectsFromList($list,$uid1);
		$final_arr = array();
		foreach($bookings as $booking){
			if(in_array($booking['status'],array(0,1,4,5,6))){
				array_push($final_arr,$booking);
			}
		}
		$booking = array();
		if(count($final_arr)>0)	$booking = $final_arr[0];
		else{
			$bookingInfo = new BookingInfo();
			$booking = $bookingInfo->getObject();
		}
		return $booking;
	}

	public function getBooking($bid,$uid){
		$booking = new Booking();
		$list = $this->cread->getList($booking,array('id'=>$bid));
		$bookings = $this->getBookingObjectsFromList($list,$uid);
		$booking = array();
		if(count($bookings)>0)	$booking = $bookings[0];
		else{
			$bookingInfo = new BookingInfo();
			$booking = $bookingInfo->getObject();
		}
	...
	function getBookingObjectsFromList($list,$uid){
		$bookings = array();
		$uids = array();
		...
		$user = new BasicUserInfo();
		$users = array();
		if(count($uids)>0)	$users = $this->cread->getUsersInfo($uids);

		foreach ($list as $booking) {
			if(isset($users[$booking['seller']]) && isset($users[$booking['customer']])){
				$bookingInfo = new BookingInfo();
				$bookingInfo->setValue('id',$booking['id']);
				$provider = $users[$booking['seller']];
				$taker = $users[$booking['customer']];

				$bookingInfo->setValue('seller',$provider['uid']);
				$bookingInfo->setValue('sellerName',$provider['name']);
				//$bookingInfo->setValue('provider_pic',$provider['pic']);

				if($provider['isCdn']==0){
					$bookingInfo->setValue('sellerPic',Constants::$VIDEO_NON_CDN_PATH . $provider['upic'] . '?alt=media');
				}
				else if($provider['thumb']!=''){
					$bookingInfo->setValue('sellerPic',Constants::$VIDEO_CDN_PATH . $provider['thumb']);
				}
				else if($provider['upic']!=''){
					$bookingInfo->setValue('sellerPic',Constants::$VIDEO_CDN_PATH . $provider['upic']);
				}

				if(isset($provider['webp']))	$bookingInfo->setValue('sellerWebp',Constants::$VIDEO_CDN_PATH . $provider['webp']);

				$bookingInfo->setValue('customer',$taker['uid']);
				$bookingInfo->setValue('customerName',$taker['name']);
				//$bookingInfo->setValue('taker_pic',$taker['pic']);

				if($taker['isCdn']==0){
					$bookingInfo->setValue('customerPic',Constants::$VIDEO_NON_CDN_PATH . $taker['upic'] . '?alt=media');
				}
				else if($taker['thumb']!=''){
					$bookingInfo->setValue('customerPic',Constants::$VIDEO_CDN_PATH . $taker['thumb']);
				}
				else if($taker['upic']!=''){
					$bookingInfo->setValue('customerPic',Constants::$VIDEO_CDN_PATH . $taker['upic']);
				}

				if(isset($taker['webp']))	$bookingInfo->setValue('customerWebp',Constants::$VIDEO_CDN_PATH . $taker['webp']);

				//if(taker]!="")	$this->datamodel['webp'] = Constants::$VIDEO_CDN_PATH . $this->datamodel['webp'];

				$bookingInfo->setValue('availability',$booking['availability']);
				$bookingInfo->setValue('locations',$booking['locations']);
				$bookingInfo->setValue('suggestedAvailability',$booking['suggestedAvailability']);
				$bookingInfo->setValue('suggestedLocations',$booking['suggestedLocations']);

				$bookingInfo->setValue('status',$booking['status']);
				if($uid==$booking['seller'])	$bookingInfo->setValue('code',$booking['sellerCode']);
				else $bookingInfo->setValue('code',$booking['customerCode']);
	...
	public function updateBookingStatus($bid,$uid,$status){
		$booking = new Booking();
		$booking = $this->getBooking($bid,$uid);
		if(isset($booking['id'])){
			$this->cwrite->updateBookingStatus($bid,$uid,$status);
			$booking['status'] = $status;
		}
		return $booking;
	}

	public function checkAndUpdateCode($bid,$uid,$code){
		$error = "";
		$booking = new Booking();
		$list = $this->cread->getList($booking,array('id'=>$bid));
		if(count($list)>0){
			$booking = $list[0];
			if(isset($booking['customer']) && $booking['customer']==$uid){
				if($code == $booking['sellerCode']){
					if($booking['status']==1 || $booking['status']==4){
						$this->cwrite->updateBookingStatus($bid,$uid,5);
					}
					else if($booking['status']==6){
						$this->cwrite->updateBookingStatus($bid,$uid,7);
					}
				}
				else{
					$error = "code don't match";
				}
			}
			else if(isset($booking['seller']) && $booking['seller']==$uid){
				if($code == $booking['customerCode']){
					if($booking['status']==1 || $booking['status']==4){
						$this->cwrite->updateBookingStatus($bid,$uid,6);
					}
		...
		$booking = $this->getBooking($bid,$uid);
		return array($booking,$error);
	}

	// public function getBookingInfo($uid1,$uid2){
	// 	$booking = new Booking();
	// 	$list1 = $this->cread->getList($booking,array('seller'=>$uid1,'customer'=>$uid2));
	// 	$list2 = $this->cread->getList($booking,array('seller'=>$uid2,'customer'=>$uid1));
	// 	$list = array_values(array_merge($list1,$list2));
	// 	//print_r($list);
	// 	$uids = array();
	// 	foreach ($list as $booking) {
	...
	// 			}
	// 			else{
	// 				$bookingInfo->setValue('sellerPic',$provider['upic']);
	// 			}
	...
	// 			}
	// 			else{
	// 				$bookingInfo->setValue('customerPic',$taker['upic']);
	// 			}
	...
	public function updateTimings($uid,$json){
		$slots_to_be_added = array();
		$slots_to_be_removed = array();
		$slot_arr = json_decode($json,true);
		$user_slots =  $this->cread->getUserSlots($uid);
		foreach($slot_arr as $slot){
			$key = $slot["dayno"] . "_" . $slot["time"];
			if($slot["issel"]=="1"){
				if(!isset($user_slots[$key]))	array_push($slots_to_be_added,$slot);
			}
			else{
				if(isset($user_slots[$key]))	array_push($slots_to_be_removed,$user_slots[$key]);
			}
		}
		if(count($slots_to_be_removed)>0) $this->cwrite->removeSlots($uid,$slots_to_be_removed);
		if(count($slots_to_be_added)>0) $this->cwrite->addSlots($uid,$slots_to_be_added);
	}

	public function updatePlaces($uid,$json){
		$places_arr = json_decode($json,true);
		foreach($places_arr as $key => $place){
			$place['address'] = addslashes($place['address']);
			$places_arr[$key] = $place;
		}
		if(count($places_arr)>0) $this->cwrite->addPlaces($uid,$places_arr);
	}
	...
	public function getBookingsToBeStarted(){
		$list = $this->cread->getBookingsToBeStarted();
		return $list;
	}
